...
1. ~~Clé API Flowstage branchée côté backend~~ — **fait.** `get_or_transcribe()` essaie maintenant, dans l'ordre : cache local → Flowstage (si `FLOWSTAGE_API_KEY` renseignée) → faster-whisper. Le matching aesthetic ↔ single se fait par comparaison floue (`find_flowstage_aesthetic()` : accents/casse/ponctuation ignorés, code numérique en tête du nom ignoré, ex. "0006 Toi & Moi", seuil de similarité + détection de sous-chaîne) — plus besoin de taper l'intitulé exact comme c'était le cas côté Make. Flowstage ne donne le timing qu'au niveau ligne (`GET /v1/aesthetics/{id}/audios`) : le texte des lignes est repris tel quel (garanti juste, zéro erreur de transcription), et le timing mot par mot est interpolé au prorata du nombre de caractères de chaque mot (`_interpolate_words()`) — précision du timing légèrement approximative sur le mot par mot, mais texte toujours exact. Testé en direct avec la vraie clé et l'aesthetic "Toi & Moi" (43 lignes / 284 mots récupérés, matching flou validé sur plusieurs variantes du titre, faux positifs écartés).
   ...
   - **Correction grammaticale via Claude (Haiku) : ajoutée, toujours active si `ANTHROPIC_API_KEY` est configurée** (décision explicite de Michel : pas d'option par job — "si je vois que ça crée trop de soucis je l'enlèverai"). `correct_lyrics_with_claude()` envoie toutes les lignes whisper (regroupées via `_words_to_phrases()`) en un seul appel avec le contexte artiste/titre, corrige uniquement les vraies erreurs de transcription (homophones, accords) sans reformuler le style/argot volontaire (consigne explicite dans le prompt — les lyrics ARK sont urbaines, le langage familier doit être préservé), et renvoie EXACTEMENT le même nombre de lignes sinon la correction est ignorée silencieusement (jamais bloquant). Pour la granularité "mot", le timing est ré-interpolé à partir des lignes corrigées via `_interpolate_words()` (même mécanique que pour les lignes Flowstage). S'applique UNIQUEMENT au repli whisper — jamais aux paroles Flowstage (déjà vérifiées humainement) ni aux corrections manuelles de Michel dans "Vérifier les paroles". Testé en local avec un client Anthropic simulé : correction propre, repli sur désaccord de nombre de lignes, et bout-en-bout mot→ligne→correction→ré-interpolation mot. **Reste à faire côté Render : ajouter la variable d'env `ANTHROPIC_API_KEY`** (même principe que Flowstage/R2) — pas encore confirmé fait par Michel. Sans cette variable, le backend continue de fonctionner normalement, juste sans la passe de correction.
